...
import fpinscala.iomonad.IO2c.{FlatMap, Return, Suspend}
...
object IO3 {
  ...
  */
  ...
  def freeMonad[F[_]]: Monad[({type f[a] = Free[F,a]})#f] = new Monad[({type f[a] = Free[F,a]})#f] {
    ...
    override def flatMap[A, B](a: Free[F, A])(f: (A) => Free[F, B]): Free[F, B] = FlatMap(a,f)
  }
  ...
  def runTrampoline[A](fa: Free[Function0,A]): A =  fa match {
    ...
    case FlatMap(x, f) => x match {
      case Return(a) => run(f(a))
      case Suspend(r) => run(f(r()))
      case FlatMap(y, g) => run(y flatMap ((a: Any) => g(a) flatMap f))
    }
  }

  // Exercise 3: Implement a `Free` interpreter which works for any `Monad`
  def run[F[_],A](freeFa: Free[F,A])(implicit F: Monad[F]): F[A] = step(freeFa) match {
  ...
  // return either a `Suspend`, a `Return`, or a right-associated `FlatMap`
  @annotation.tailrec
  def step[F[_],A](freeFa: Free[F,A]): Free[F,A] = freeFa match {
    case FlatMap(FlatMap(x, f), g) => step(x flatMap (a => f(a) flatMap g))
    case FlatMap(Return(x), f) => step(f(x))
    case _ => freeFa
  }
