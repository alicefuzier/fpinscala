...
trait Monad[M[_]] extends Functor[M] {
  ...
  def sequence[A](lma: List[M[A]]): M[List[A]] = lma.foldRight(unit(List[A]()))((ma,mListA)=> map2(ma,mListA)((a,listA)=> a :: listA) )

  def traverse[A,B](la: List[A])(f: A => M[B]): M[List[B]] = la.foldRight(unit(List[B]()))((a,mListB)=> map2(f(a),mListB)(_ :: _))

  def replicateM[A](n: Int, ma: M[A]): M[List[A]] = if (n <= 0) unit(List[A]())
    else map2(ma, replicateM(n-1, ma))(_ :: _)

  def filterM[A](ms: List[A])(f: A => M[Boolean]): M[List[A]] = ms.foldRight(unit(List[A]()))((a,mListA) => map2(f(a),mListA)((bol,listA)=>if(bol) a:: listA else listA))
...
object Monad {
  ...
  def parserMonad[C](p: Parsers[Parser[C]]): Monad[Parser] = new Monad[Parser] {
    override def unit[A](a: => A): Parser[A] = p.succeed(a)

    override def flatMap[A, B](ma: Parser[A])(f: (A) => Parser[B]): Parser[B] = p.flatMap(ma)(f)
  }
